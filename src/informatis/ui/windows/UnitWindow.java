...
public class UnitWindow extends Window {
    ...
    public static class BarData extends SBar.SBarData {
        ...
        public BarData(String name, Color color, float fraction, TextureRegion icon) {
            this(name, color, () -> fraction, icon);
        }

        public BarData(String name, Color color, Floatp fraction, TextureRegion icon) {
    ...
    static class UnitWindowBody extends Table implements IRebuildable {
        ...
        private Table buildContent(Teamc target) {
            return new Table(Styles.black8, table -> {
                ...
                table.image().color((target == null ? player.unit() : target).team().color).height(4f).pad(4f);
                table.row();
                table.table(bars -> {
                    Seq<BarData> barDataSeq = new Seq<>();
                    barDataSeq.clear();
                    getInfo(target, barDataSeq);
                    int initialSize = barDataSeq.size;

                    bars.update(() -> {
                        barDataSeq.clear();
                        getInfo(target, barDataSeq);
                    });
                    SUtils.loop(initialSize, i -> {
                        bars.table(bar -> {
                            SBar sbar = new SBar(barDataSeq.get(i));
                            BarIconImage barIcon = new BarIconImage(barDataSeq.get(i));

                            bar.update(() -> {
                                BarData barData = barDataSeq.get(i);
                                sbar.barData = barData;
                                barIcon.barData = barData;
                            });
